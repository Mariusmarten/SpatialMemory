...
class ObtainDualDataset(Dataset):
    ...
    def __init__(
        self, dic, imagesA, imagesB, labels, transform=None, target_transform=None
    ):
        self.img_labels = dic[labels]
...
def split_data_for_trajectories(
    data, train_set_perc=0.8, shuffle=True, length_trajectory=10
):
    """
    Splits dataset into training and test set.
    Returns training and test set.
...
def recode_actions(dataset, n):
    ...
    for actions in dataset["actions"]:
        actions_recoded.append(
            [single_class_encoding_dic[str(actions)]]
        )  # recode all actions
