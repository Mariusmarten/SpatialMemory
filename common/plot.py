...
def plot_3x3_examples(dic):
    ...
    for i in range(1, columns * rows + 1):
        ...
        if i in [1, 4, 7]:
            rand = random.randint(
                0, len(dic["actions"]) - 1
            )  # generate random number to pick image
...
def plot_acc(test_acc, train_acc, smooth=False):
    ...
    if smooth:
        test_acc_smoothed = savgol_filter(
            test_acc, 30, 12
...
def show_example_classificataions(dataset, net, amount=5):
    ...
    print(
        "GroundTruth: ",
        ", ".join(f"{classes_expl[int(labels[j])]}" for j in range(amount)),
    )
    print(
        "Predicted: ",
        ", ".join(f"{classes_expl[int(predicted[j])]}" for j in range(amount)),
        "\n",
    )
...
def plot_confusion_matrix(dataset, network, save=False):
    ...
    df_cm = pd.DataFrame(
        ...
        index=[classes_expl[i] for i in classes],
...
def histo_distribution_shift(test_dis_item):
    ...
    with plt.style.context("ggplot"):
        plt.hist(
            dis_item_notensor[0 : int(length * 0.1)],
            bins=100,
            rwidth=0.9,
            color="indianred",
            range=(0, max_lim),
    ...
    with plt.style.context("ggplot"):
        plt.hist(
            dis_item_notensor[int(length * 0.9) : length - 1],
            bins=100,
            rwidth=0.9,
            color="indianred",
            range=(0, max_lim),
...
def histo_train_val(test_dis_item, train_dis_item):
    ...
    with plt.style.context("ggplot"):
        ...
        plt.text(
            mean_val * 1.1, max_lim * 0.9, "Mean: {:.2f}".format(mean_val), fontsize=22
        )
        plt.axvline(mean_val, color="k", linestyle="dashed", linewidth=2)
        plt.show()
